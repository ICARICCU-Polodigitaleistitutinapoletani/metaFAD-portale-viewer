...
(function() {
  ...
  /** @ngInject */
    ...
    function metaMediaViewerController($rootScope, $scope, $log, $sce, $element, $filter, $timeout, refreshAngularCircle, DamCollectionManagerService, viewerFactory, MainService, mergeArrayUnique, serviceUUID, LoadingService, $uiMrFeedback) {
        ...
        vm.contentHeightNoTollbar = parseInt(vm.contentHeight)-38 + "px";
        ...
        var setDoublePage = function(){
            ...
            if(!posMedia || posMedia===0){
                if(!allMedia[indexMedia+1]){
                    getMedia(indexMedia+1,"setDoublePage");
                    return;
                }
                vm.doubleMedia[0] = vm.media;
                vm.doubleMedia[1] = allMedia[indexMedia+1];
            }
            else{
                if(!allMedia[indexMedia-1]){
                    getMedia(indexMedia-1,"setDoublePage");
                ...
                vm.doubleMedia[1] = vm.media;
                vm.doubleMedia[0] = allMedia[indexMedia-1];
            }
            vm.bodyHeight=angular.element($element.find(".body")).height();
        };
        ...
        vm.setMedia = function(val,dir){
            ...
            vm.infoMedia = vm.infoMedia ? vm.infoMedia +'<br/>' + vm.media.title : vm.media.title || '';
            if(!vm.media){
            ...
            if(vm.media.tile){
                ...
                if(METAFAD_VIEWER_CONFIG.navigatorBlock){
                    ...
                    $(".mibac_museowebfad_viewer .displayregion").attrchange({
                        trackValues: true, // set to true so that the event object is updated with old & new values
                        callback: function(evnt) {
                            if(evnt.attributeName == "style") { 
                                var w = $(".navigator canvas").width()-parseInt(evnt.target.style.width);
                                var h = $(".navigator canvas").height()-parseInt(evnt.target.style.height);
        ...
        vm.rotate = {
            ...
            "setRotate":function(deg){
                var img = $(".box-img-notile img");
                var transImg = $(".box-img-notile img")[0].style.transform;
                ...
                $(".box-img-notile img")[0].style.transform = transImg;
            },
            "right":function(){
